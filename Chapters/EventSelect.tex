...
There are three steps to created simulated events. The first step is the 
...
the M\textsc{AD}G\textsc{RAPH}5~\cite{Alwall:2014hca} matrix-element generator was used.

The second step is to take the list of partons and corresponding 
momentum 4-vectors from the output of the event generator and 
perform parton showering. The partons are randomly fragmented into
multiple partons until the energy of each parton is below a set threshold. 
The partons are then hadronized. Additionally, the generation of initial state radiation
(ISR) occurs during this step. 
We used the general-purpose event generator 
\textsc{PYTHIA}8~\cite{Sjostrand:2007gs}.
...
rely on the G\textsc{EANT}4 program.
...
detector hist. The final simulated output ends up in the same format as the output of the 
data reconstruction.
