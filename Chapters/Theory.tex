...
The immediate consequence is that the number of elementary particles is at least double that in the standard model. For each SM boson, there will be a fermion ``superpartner" and vice versa. For all its apparent simplicity, Equation~\ref{equ:SUSYdef} has many implications that provide attractive solutions to the issues with the SM listed above.
...
From the lack of experimental observation, it must be true that the masses of the messenger particles must be very high. 
...
\begin{figure*}[htbp]
    ...
    Reprinted from Reference~\cite{SUSYprimer}.}
    \label{fig:GMSBscalar}
\end{figure*}

\begin{figure*}[htbp]
    \centering
    \includegraphics[width=0.3\textwidth]{Figures/Theory/GMSBgaugino.pdf}
    \caption{Example Feynman diagram illustrating how the messenger particles in SUSY models
...
restarted at an even higher center-of-mass-energy, $\sqrt{s} = 13$ TeV. The jump in energy corresponded
...
data set, from 12.9 \fbinv to 35.9 \fbinv. See Reference~\cite{pdgReview} for an overview of SUSY 
...
\begin{figure*}[htbp]
    ...
    \caption{ Excluded mass ranges for a selection of CMS results using the 2016 data set \cite{SUSYtwiki}.}
...
which will be described in detail in this dissertation, particularly in Chapter~\ref{chap:DataAnalysis}. 
